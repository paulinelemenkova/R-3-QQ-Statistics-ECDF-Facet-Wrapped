...
# шаг-4. чертим график регрессионного анализа по MDepths_df
MDepths_df$profiles<- as.factor(MDepths_df$profiles) #задаем номера профилей как факторное значение
head(MDepths_df) 

# одиночный график 1 вариант, через qplot
p<- qplot(data = MDepths_df, sample = MDepths_df$depths, color=MDepths_df$profiles) 

# одиночный график 2 вариант через ggplot плохой: не контролирует размер точечек ! но работает.
...
# 3 множественный фасетный график по статистике QQ из 25 маленьких 	
p_tr<- qplot(sample = MDepths_df$depths, data = MDepths_df, color = profiles, size = profiles) + 
	stat_qq() +  
	geom_line(aes(color = profiles)) +
	facet_wrap( ~ profiles) +
	theme(legend.position = "top") + 
...
#подписываем текст-аннотации на разные графики
p_ann <- p_tr + geom_point() 
p_ann1 <- p_ann + facet_grid( ~ profiles) 
p_ann2 <- p_ann1 + annotate("text", label = "Polina", size = 4, x = 1, y = -1000) 

  	
# добавляем обертку темы (легенда, оси, подписи и пр.)
QQ_facetwrapMD <- p_tr + 
	geom_abline(intercept = mean(MDepths_df$depths), slope = sd(MDepths_df$depths), colour = "red", size = .3, show.legend = TRUE) +
	geom_segment(aes(x = 1, y = -7500, xend = 0, yend = -5000), size = .1, arrow = arrow(length = unit(0.2, "cm"))) + 
	annotate("text", x = 1, y = 7500, label= "boat") +   
	theme(
...
#ggsave("QQ_facetwrapMD.pdf", device = cairo_pdf, fallback_resolution = 300, width = 210, height = 297, units = "mm") # сохраняем на размер A4 (именно сама картинка будет растянута на A4)
